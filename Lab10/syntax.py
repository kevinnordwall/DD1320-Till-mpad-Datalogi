...
def weight(mol):
    if not mol:
        return 0
    if mol.down:
        sum_weight = mol.num * weight(mol.down)
        if mol.next:
            return sum_weight + weight(mol.next)
        else:
            return sum_weight
    else:
        atoms = atom.main()
        return atoms[mol.atom]*mol.num + weight(mol.next)
...
def main():
    #stdin = open(file_name1) #Lätt att ändra för att testa indata från fil
    for rad in stdin:
